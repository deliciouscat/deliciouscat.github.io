...
## Facade, Observer Pattern과 차이

Mediator는 특히 Facade, Observer와 혼동하기 쉽다. 모두 결합도를 낮추지만, 해결하려는 문제가 다르다.
...
**Observer**는 Subject의 상태 변화를 등록된 Observer 모두에게 알리는 **일대다 알림** 패턴이다. Subject가 Observer 목록을 관리하며, 이벤트 리스너나 주가 알림에 적합하다.
...
또한 채팅방 예제만 보면 Mediator가 메시지를 여러 사용자에게 보내므로 Observer처럼 보일 수 있다. 하지만 핵심은 채팅방이 단순히 알림을 뿌리는 것이 아니라, 참여자·수신 범위·권한·메시지 형식을 **중앙에서 조율**한다는 데 있다.
